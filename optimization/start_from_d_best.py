...
def _append_row(ds, row):
    ...
    if ds.ndim == 1:
        ...
        # Logic to handle variable width (number of edges)
        current_rows, current_width = ds.shape
        new_width = row.shape[0]

        # 1. If new row is WIDER than dataset, resize dataset columns
        if new_width > current_width:
            ds.resize(new_width, axis=1)
            current_width = new_width

        # Resize vertical (add new row)
        ds.resize(current_rows + 1, axis=0)

        # 2. If new row is NARROWER than dataset, pad with zeros
        if new_width < current_width:
            # Create a padded array matching the dataset width
            padded = np.zeros(current_width, dtype=ds.dtype)
            padded[:new_width] = row
            ds[-1, :] = padded
        else:
            ds[-1, :] = row
...
def init_dsets(grp, initial_state, initial_result):
    ...
    dsets = {}
    dsets["states"] = _ensure_ds(grp, "states",               parse_edgelist(
        initial_state).astype(np.uint32), is_row=True)
    dsets["logical_error_rates"] = _ensure_ds(grp, "logical_error_rates",  np.array(
        float(initial_result['logical_error_rates'][0]), dtype=np.float64), is_row=False)
    dsets["logical_error_rates_std"] = _ensure_ds(grp, "logical_error_rates_std", np.array(
        float(initial_result['stderrs'][0]), dtype=np.float64), is_row=False)

    dsets["n_classical"] = _ensure_ds(grp, "n_classical",          np.array(
        int(initial_result['n_classical']), dtype=np.int32), is_row=False)
    dsets["n_classical_T"] = _ensure_ds(grp, "n_classical_T",        np.array(
        int(initial_result['n_T_classical']), dtype=np.int32), is_row=False)
    dsets["k_classical"] = _ensure_ds(grp, "k_classical",          np.array(
        int(initial_result['k_classical']), dtype=np.int32), is_row=False)
    dsets["k_classical_T"] = _ensure_ds(grp, "k_classical_T",        np.array(
        int(initial_result['k_T_classical']), dtype=np.int32), is_row=False)
    dsets["rank_H"] = _ensure_ds(grp, "rank_H",               np.array(
        int(initial_result['rank_H']), dtype=np.int32), is_row=False)
    dsets["skipped"] = _ensure_ds(grp, "skipped",              np.array(
        int(initial_result['skipped']), dtype=np.int32), is_row=False)

    dsets["distances_classical"] = _ensure_ds(grp, "distances_classical",   np.array(
        float(initial_result['d_classical']), dtype=np.float64), is_row=False)
    dsets["distances_classical_T"] = _ensure_ds(grp, "distances_classical_T", np.array(
        float(initial_result['d_T_classical']), dtype=np.float64), is_row=False)
    dsets["distances_quantum"] = _ensure_ds(grp, "distances_quantum",     np.array(
    ...
    dsets["beam_survivors"] = _ensure_ds(
        grp, "beam_survivors", sample_beam_row, is_row=True)

    dsets["parent_idx"] = _ensure_ds(
        grp, "parent_idx", np.array(-1, dtype=np.int32), is_row=False)
...
def append_record(dsets, state, result, parent_id):
    ...
    ler = float(result['logical_error_rates'][0]) if len(
        result['logical_error_rates']) else 0.0
    ...
    _append_row(dsets["logical_error_rates_std"],
                np.array(lstd, dtype=np.float64))

    _append_row(dsets["n_classical"],   np.array(
        int(result['n_classical']), dtype=np.int32))
    _append_row(dsets["n_classical_T"], np.array(
        int(result['n_T_classical']), dtype=np.int32))
    _append_row(dsets["k_classical"],   np.array(
        int(result['k_classical']), dtype=np.int32))
    _append_row(dsets["k_classical_T"], np.array(
        int(result['k_T_classical']), dtype=np.int32))
    _append_row(dsets["rank_H"],        np.array(
        int(result['rank_H']), dtype=np.int32))
    _append_row(dsets["skipped"],       np.array(
        int(result['skipped']), dtype=np.int32))

    _append_row(dsets["distances_classical"],   np.array(
        float(result['d_classical']), dtype=np.float64))
    _append_row(dsets["distances_classical_T"], np.array(
        float(result['d_T_classical']), dtype=np.float64))
    _append_row(dsets["distances_quantum"],     np.array(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-L', action="store", dest='L',
                        default=None, type=int, required=False)
    ...
    with h5py.File(output_file, "a") as f:
        ...
        if "best_state" in grp:
            del grp["best_state"]
        grp.create_dataset(
            "best_state", data=best_state_edgelist[np.newaxis, :], dtype=np.uint32)
        ...
        print(
            f"Initial Cost: {logical_error_rate:.6f}, Distance Threshold: {distance_threshold}")
        ...
        for l in range(L):
            ...
            print(
                f"Iteration {l+1}/{L} | Beam Size: {len(current_beam)} | Neighbors per parent: {n_per_parent}")
            ...
            for parent_idx, parent_info in enumerate(current_beam):
                ...
                print(
                    f"  >>> [Parent {parent_idx+1}/{len(current_beam)} ({parent_row_idx})] Starting scan...")
                ...
                for n in range(n_per_parent):
                    print(
                        f"    - Scanning neighbor {n+1}/{n_per_parent} of parent {parent_idx+1} ({parent_row_idx}) in Iteration {l+1}...")
                    scanned_total += 1
                    ...
                    ler = float(cost_result['logical_error_rates'][0]) if len(
                        cost_result['logical_error_rates']) else 1.0
                    dist_any = min(float(cost_result['d_classical']), float(
                        cost_result['d_T_classical']))
                    ...
                    if not skipped and ler < min_cost:
                        ...
                        if "best_state" in grp:
                            del grp["best_state"]
                        best_state_edgelist = parse_edgelist(
                            neighbor).astype(np.uint32)
                        grp.create_dataset("best_state", data=best_state_edgelist, shape=(
                            1, best_state_edgelist.shape[0]), dtype=np.uint32)
                ...
                print(
                    f"  <<< [Parent {parent_idx+1}] Finished. Found {valid_found_this_parent} valid.")
            ...
            if candidate_pool:
                ...
                sources = ["(Parent)" if c['row_idx'] <
                           step_start_row else "(Child)" for c in top_k]
                print(
                    f"  Selection: Keeping {len(top_k)} best. Costs: [{costs_str}]")
                print(f"  Sources: {sources}")
                print(f"  Distances: {[int(c['dist']) for c in top_k]}")

                distance_threshold = max(
                    top_k, key=lambda x: x['dist'])['dist']

            else:
                # Fallback logic
                print(
                    "  No valid neighbors found. Attempting fallback (max distance)...")
                if fallback_candidates:
                    ...
                    if best_fallback['result'].get('skipped', False):
                        ...
                        best_fallback['row_idx'] = _current_stream_len(
                            dsets) - 1

                    next_beam = [best_fallback]
                    distance_threshold = int(best_fallback['dist'])
                    print(
                        f"    Fallback selected. New Threshold: {distance_threshold}")
                else:
            ...
            _append_row(dsets["beam_survivors"], np.array(
                survivor_indices, dtype=np.int64))
            _append_row(dsets["step_summaries"], np.array(
                [step_start_row, scanned_total, len(valid_candidates), survivor_indices[0]]))
        ...
        print(
            f"Total runtime: {grp.attrs['total_runtime'] // 3600} hours {(grp.attrs['total_runtime'] % 3600) // 60} minutes {grp.attrs['total_runtime'] % 60} seconds")
